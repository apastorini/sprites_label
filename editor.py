...
class SpritesheetSelectorApp:
    def __init__(self, master):
        self.master = master
        master.title("Spritesheet Editor v2.1")
    ...
    def delete_selected_sprite(self):
        ...
        if not selected_names or not messagebox.askyesno("Confirmar",
                                                         f"¿Eliminar {len(selected_names)} sprite(s) seleccionado(s)?"):
            return
    ...
    def suggest_sprites(self):
        ...
        if self.get_all_sprites() and not messagebox.askyesno("Confirmar",
                                                              "Esto borrará los sprites actuales. ¿Continuar?"):
            return
        ...
        count = 0
        # --- MEJORA: Bucle de sugerencia ordenado ---
    ...
    def save_json(self):
        ...
        if file_path:
            ...
            try:
                # Intenta obtener una ruta relativa
                relative_path = os.path.relpath(self.project_data['spritesheet_path'], json_dir)
            except ValueError:
                # Si están en unidades diferentes (ej. C: y D:), usa la ruta absoluta
                relative_path = self.project_data['spritesheet_path']

            serializable_data = self.project_data.copy()
            # Guardamos solo el nombre del archivo, asumiendo que estará en la misma carpeta que el JSON
